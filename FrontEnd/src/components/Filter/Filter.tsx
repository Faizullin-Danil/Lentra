...
const Filter: React.FC<FilterProps> = ({ whichPage }) => {
  ...
  const filterByYears = () => {
    ...
    const isFilmsYears = allFilms.filter(
      (film) => film.year >= Number(valueFrom) && film.year <= Number(valueTo)
    );
    const isFavouritesFilmsYears = allFavouritesFilms.filter(
      (film) => film.year >= Number(valueFrom) && film.year <= Number(valueTo)
    );
    ...
    if (!hasError) {
      ...
      if (whichPage === "Главная страница" && isFilmsYears.length !== 0) {
        ...
        whichPage === "Страница избранных" &&
        isFavouritesFilmsYears.length !== 0
      ) {
          ...
          <TextField
            ...
            sx={{
              ...
              "& .MuiOutlinedInput-root": {
                ...
                "&.Mui-error fieldset": {
                  borderColor: errorFrom ? "red" : "black",
                  borderWidth: errorFrom ? 2 : 1,
                },
              },
            }}
          />
          <TextField
            ...
            sx={{
              ...
              "& .MuiOutlinedInput-root": {
                ...
                "&.Mui-focused fieldset": { borderColor: "black" },
                "&.Mui-error fieldset": {
                  borderColor: errorTo ? "red" : "black",
                  borderWidth: errorTo ? 2 : 1,
                },
              },
            }}
          />
