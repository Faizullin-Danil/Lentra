...
exports.fetchAndSaveImagesByMovies = async () => {
    try {
        ...
        for (const kinopoiskId of kinopoiskIdsArray) {
            try {
                const response = await axios.get(`https://kinopoiskapiunofficial.tech/api/v2.2/films/${kinopoiskId}/images?type=STILL&page=1`, {
                            headers: {
                                'X-API-KEY': API_KEY,
                            }
                        });
                ...
                console.error('Ошибка в цикле сервиса images в fetchAndSaveImagesByMovies:', error.message);
            }
        }
